...
const ToiletVideoLightbox = ({
  ...
  useEffect(() => {
    if (userId) {
      const userIsLiked = updatedVideo?.likes.includes(userId);
      if (userIsLiked) {
        setIsLiked(true);
      } else {
        setIsLiked(false);
  ...
  const { mutateAsync: LikeToiletMultimedia } = useMutation({
    ...
    onSuccess: (res) => {
      ...
      setUpdatedVideo(res.data.selectedMultimedia);
    },
    onError: (err) => {
    ...
    <Overlay>
      <Lightbox styles="mt-8" onCloseLightbox={onCloseLightbox}>
        <div className="flex flex-col gap-y-5 justify-center items-center px-5">
          {updatedVideo && (
            <div className="relative">
              <video
                ...
                autoPlay
              >
                <source src={updatedVideo?.url} type="video/mp4" />
                <source src={updatedVideo?.url} type="video/webm" />
          ...
          <div className="flex flex-row-reverse gap-x-3 w-full h-full md:px-12">
            <CustomButton
              type="button"
            ...
            <div className="flex gap-x-2 items-center">
              ...
              />
              <p className="text-xs">{updatedVideo.comments.length}</p>
            </div>
            ...
            <div className="flex gap-x-2 items-center">
              ...
              />
              <p className="text-xs">{updatedVideo?.likes.length}</p>
            </div>
          </div>
        </div>
